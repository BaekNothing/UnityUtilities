...
public class OptimizedScrollview : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    float cap_Hard_ContentPosValue = 70f;
    ...
    {
        inertiaForce = Mathf.Min(inertiaForceCap, ((oneFramePrevPos - nowPos) * scrollDir).magnitude);
        inertiaForce = Mathf.Max(-inertiaForceCap, inertiaForce);
        inertiaDir = ((oneFramePrevPos - nowPos) * scrollDir).normalized;
        originPos = null;
        PositionRearrangement();
    }
    ...
    private void LateUpdate()
    {
        ...
        if (originPos != null)
        {
            ...
            if ((inertiaForce -= Time.deltaTime) <= 0)
            {
                oneFramePrevPos = nowPos;
                inertiaForce = 0.05f;
            }
        }
        else if(inertiaForce > 0)
            SetInertia();
        else
            RecoveryPos();
    }

    Vector2 oneFramePrevPos;
    Vector2 inertiaDir;
    float inertiaForce = 0f;
    float inertiaForceCap = 50f;

    protected void SetInertia()
    {
        content.anchoredPosition += (isReverse ? 1 : -1) *
            inertiaDir * inertiaForce * 30 * Time.deltaTime;
        inertiaForce -= Time.deltaTime * inertiaForce;
        inertiaForce--;
